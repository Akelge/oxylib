...
def HTMLPrint(what, title="Unknown Object"):
    ...
    output = []
    output.append(
        "<table border='2' width=80%% align='center'><tr><td align='center' colspan='2'><b>%s</b></td></tr>" % title)
    if hasattr(what, 'items'):
        for k, v in sorted(what.items()):
            output.append("<tr><td><b>%s</b></td><td>%s</td></tr>" %
                          (k, html_escape(repr(v))))
    else:
        output.append(
            "<tr><td colspan='2'>%s</td></tr>" % html_escape(type(what)))
...
def printConfig(config):
    return HTMLPrint(config, "Config")


def printSession(session):
    return HTMLPrint(session, "Session")


def printCGIEnv():
    return HTMLPrint(dict(([i for i in request.environ.items() if i[0].upper() == i[0]])), "CGI Environ")


def printWSGIEnv():
    return HTMLPrint(dict(([i for i in request.environ.items() if i[0].lower() == i[0]])), "WSGI Environ")
