...
# __all__ = ["BaseUser", "AuthenticatedUser", "login", "logout",
# "isAuthenticated", "isAuthorized", "authorize"]

import time
import hashlib  # for _uuid
import re
...
class BaseUser(object):
    ...
    def isAuthorized(self, clause=None):
        return isAuthorized(clause=clause, user=self)
    ...
    def __str__(self):
        "username: %s - login_time: %s - AUTH: %s" % (
            self.username, self.login_time, self.authenticated)
    ...
    def avPairs(self):
        ...
        """
        d = sdict()
        for k in [key for key in self.__dict__.keys() if not key.startswith('_')]:
            d[k] = self.__dict__[k]
...
class AuthenticatedUser(BaseUser):
    ...
    def _doAcctPlugins(self, start=False, stop=False):
            ...
            for module in self._ACCOUNTING_MODULES:
                ...
                try:
                    self.updateFromPlugin(
                        self._REGISTERED_PLUGINS["startAcct_%s" % (module)](self.avPairs))
                except KeyError:
        ...
        if stop:
            for module in self._ACCOUNTING_MODULES:
                try:
                    self.updateFromPlugin(
                        self._REGISTERED_PLUGINS["stopAcct_%s" % (module)](self.avPairs))
                except KeyError:
    ...
    def authz(self):
        self._doAuthzPlugins()

    def startAcct(self):
        self._doAcctPlugins(start=True, stop=False)

    def stopAcct(self):
        self._doAcctPlugins(start=False, stop=True)
...
def isAuthorized(clause=None, user=None, sep='.'):
    ...
    if clause:
        ...
        try:
            ...
            raise Exception("Invalid clause! - Clause: \"%s\" - Expanded: \"%s\"" % (
                clause, expandedclause))
    else:
...
def isAuthenticated(user=None):
    ...
    if not user:
        user = sessionLoad()
    return user.authenticated is True
