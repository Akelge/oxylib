        ...
        if args:
            if type(args[0]) is DictType:
                ...
                kwargs.update(args[0].__dict__)
                         ...
                         '__class__') and args[0].__class__.__name__ == 'sdict':
...
class kstruct(object):
    ...
    def __getattr__(self, attr):
        if attr in self.__dict__['_elements']:
    ...
    @property
    def dict(self):
        return self.__dict__['_elements']
...
class kstructElement(object):
    ...
    def __repr__(self):
        return "<%s \"%s\" -> %d >" % (self.__class__.__name__, self.key, self.value)
